...
class HelpView(LayoutView):
    def __init__(self, dev_user: Member, rules_channel_url: str, timeout: int = 180) -> None:
        ...
        super().__init__(timeout=timeout)
        text = ("""**\nCOMMANDS**\n**/bottekin stats**\nDisplays the given user's statistics\nIf the user option is not provided, it displays statistics of the user who called the command.\n
        ...
        "\n**RATE LIMITS**\n**/bottekin stats:** You can use this command up to 3 times every 5 minutes.\n**/bottekin make_it_quote:** You can use this command up to 5 times every 24 hours."
        f"**\nCONTACT**\nYou can send an email to aiden8hunter@gmail.com\nAnd feel free to send a dm to me {self.dev_user.mention}\n"
        f"**\nPRIVACY**\nThis bot cannot access any private data or does not store any private data.\nThe stored data are:\nYour public user id and username provided by discord API\nTrack titles you shared in the server\nFeedback message content and statistics of shared tracks, feedback messages, challenges you participated&won bot stores these data to present the statistics to you and the other members.\n**By default, the bot is set to keep your data in database after you leave the server.**\n**Optionally**, you can set the bot to delete all your data by adding a :x: to the message on {self.rules_channel_url}.This way, the bot will delete all your data when you leave the server."
        "\n**Important**\nIf you react with :x: to the rule message and leave the server, the bot will purge all data related to you. However, please note that the bot cannot delete your messages from Discord channels after you leave. Those messages will remain visible, but they will be ignored by the bot.")
